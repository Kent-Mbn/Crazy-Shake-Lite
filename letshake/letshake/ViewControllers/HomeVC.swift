...
class HomeVC: UIViewController, UIAlertViewDelegate, UIActionSheetDelegate, UITableViewDataSource, UITableViewDelegate {
    ...
    @IBAction func actionTopLeft(sender: AnyObject) {
        ...
        /*
        let actionSheetMore : UIActionSheet
        if UserDefault.shareIntance!.isLogin {
            actionSheetMore = UIActionSheet(title: APP_NAME, delegate: self, cancelButtonTitle: "Cancel", destructiveButtonTitle: nil, otherButtonTitles: "Help", "Select sound", "Logout")
            actionSheetMore.tag = 1
        } else {
            actionSheetMore = UIActionSheet(title: APP_NAME, delegate: self, cancelButtonTitle: "Cancel", destructiveButtonTitle: nil, otherButtonTitles: "Help", "Select sound")
            actionSheetMore.tag = 2
        }
    ...
    func tableView(tableView: UITableView, didSelectRowAtIndexPath indexPath: NSIndexPath) {
        ...
        if tableView == self.tblSelectSound {
            ...
            if (isLite && indexPath.row > 1) {
                Common.showAlertView(APP_NAME, message: Message.sharedInstance.MSS_DOWNLOAD_PRO_VERSION, delegate: self, cancelButtonTitle: "Later", arrayTitleOtherButtons: ["Buy now"], tag: 2)
            } else {
        ...
        if tableView == self.tblMorePopup {
            ...
            case 3:
                UIApplication.sharedApplication().openURL(NSURL(string: urlAppStore)!)
            case 4:
                if UserDefault.shareIntance!.isLogin {
                    Common.showAlertView(APP_NAME, message: Message.sharedInstance.MSS_LOGOUT_SURE, delegate: self, cancelButtonTitle: "No", arrayTitleOtherButtons: ["Yes"], tag: 1)
                } else {
